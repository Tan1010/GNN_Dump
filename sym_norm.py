...
# # g = dgl.graph(([1,2,3], [0,1,2]), num_nodes=4)
# g = dgl.graph(([0,1,2,3,2,5], [1,2,3,4,0,3]))

# g = dgl.add_self_loop(g)
# feat = th.tensor([[1.0,1.0,1.0],[2.0,2.0,2.0], [3.0,3.0,3.0], [4.0,4.0,4.0], [5.0,5.0,5.0], [6.0,6.0,6.0]])
# # feat = th.ones(6, 3)
# print(f"Feat: {feat}")
# # edge_weight = th.tensor([1, 1, 1, 1, 1, 1, 1, 1])
# edge_weight = th.tensor([1, 1, 1, 1, 1, 1.0, 1, 1, 1, 1, 1, 1])
# norm = EdgeWeightNorm(norm='both')
# norm_edge_weight = norm(g, edge_weight)
# conv = GraphConv(3, 3, norm='none', weight=False, bias=False)
...
class GraphConv(nn.Module):
    ...
    def __init__(self,
        ...
        if norm not in ('none', 'both', 'right', 'left'):
            raise DGLError('Hello. Invalid norm value. Must be either "none", "both", "right" or "left".'
                           ' But got "{}".'.format(norm))
...
class GCN(nn.Module):
    ...
    def forward(self, g, in_feat):
        ...
        feat_src, feat_dst = expand_as_pair(in_feat, g)
        ...
        shp = norm.shape + (1,) * (feat_src.dim() - 1)
        norm = th.reshape(norm, shp)
        print(f"\nFeat_src 1.1= {feat_src}")
        feat_src = feat_src * norm
        print(f'\nNorm: {norm}')
        print(f"\nFeat_src 1.2= {feat_src}")
        g.srcdata['h'] = feat_src
        aggregate_fn = fn.copy_src('h', 'm')
        g.update_all(aggregate_fn, fn.sum(msg='m', out='h'))
