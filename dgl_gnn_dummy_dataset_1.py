...
for i in range(k):
    ...
    #     exit()
    # train
    no_of_nodes = len(current_fold_train_labels)
    half = no_of_nodes//2
    no_positive_features = half
    no_negative_features = no_of_nodes - half
    dummy_positive_features = np.random.uniform(low=0.5, high=1, size=(no_positive_features,))
    dummy_negative_features = np.random.uniform(low=0, high=0.5, size=(no_negative_features,))
    dummy_positive_features = np.reshape(dummy_positive_features, (len(dummy_positive_features), 1))
    dummy_negative_features = np.reshape(dummy_negative_features, (len(dummy_negative_features), 1))
    dummy_positive_labels = np.ones((dummy_positive_features.shape))
    dummy_negative_labels = np.zeros((dummy_negative_features.shape))
    
    dummy_features = np.concatenate((dummy_positive_features, dummy_negative_features))

    dummy_labels = np.zeros(no_of_nodes)
    np.random.shuffle(dummy_features)
    for j in range(len(dummy_features)):
        if dummy_features[j] >= 0.5:
            dummy_labels[j] = 1.0
        else:
            dummy_labels[j] = 0.0


    validation_no_of_nodes = len(current_fold_validation_labels)
    validation_half = validation_no_of_nodes//2
    validation_no_positive_features = validation_half
    validation_no_negative_features = validation_no_of_nodes - validation_half
    validation_dummy_positive_features = np.random.uniform(low=0.5, high=1, size=(validation_no_positive_features,))
    validation_dummy_negative_features = np.random.uniform(low=0, high=0.5, size=(validation_no_negative_features,))
    validation_dummy_positive_features = np.reshape(validation_dummy_positive_features, (len(validation_dummy_positive_features), 1))
    validation_dummy_negative_features = np.reshape(validation_dummy_negative_features, (len(validation_dummy_negative_features), 1))
    validation_dummy_positive_labels = np.ones((validation_dummy_positive_features.shape))
    validation_dummy_negative_labels = np.zeros((validation_dummy_negative_features.shape))
    
    validation_dummy_features = np.concatenate((validation_dummy_positive_features, validation_dummy_negative_features))

    validation_dummy_labels = np.zeros(validation_no_of_nodes)
    np.random.shuffle(validation_dummy_features)
    for j in range(len(validation_dummy_features)):
        if validation_dummy_features[j] >= 0.5:
            validation_dummy_labels[j] = 1.0
        else:
            validation_dummy_labels[j] = 0.0
    
    train_graph= dgl.graph((current_fold_train_list1, current_fold_train_list2), num_nodes = len(current_fold_train_labels))
    ...
    validation_graph= dgl.graph((current_fold_validation_list1, current_fold_validation_list2), num_nodes = len(current_fold_validation_labels))
    ...
    print(f'Total train nodes: {len(dummy_labels)}')
    print(f'Total val nodes: {len(validation_dummy_labels)}')
    
    # figure, axis = plt.subplots(1,1)
    # sns.histplot(data=np.array(dummy_features), kde=False, stat='density')
    # sns.kdeplot(data=np.array(dummy_features), color='crimson')
    # plt.xlabel('x')
    # plt.ylabel('y')
    # plt.title('Dummy Training Features')
    # plt.draw()

    # figure, axis = plt.subplots(1,1)
    # sns.histplot(data=np.array(validation_dummy_features), kde=False, stat='density')
    # sns.kdeplot(data=np.array(validation_dummy_features), color='crimson')
    # plt.xlabel('x')
    # plt.ylabel('y')
    # plt.title('Dummy Validation Features')
    # plt.draw()
...
testing_no_of_nodes = len(node_labels)
testing_half = no_of_nodes//2
testing_no_positive_features = testing_half
testing_no_negative_features = testing_no_of_nodes - testing_half
testing_dummy_positive_features = np.random.uniform(low=0.5, high=1, size=(testing_no_positive_features,))
testing_dummy_negative_features = np.random.uniform(low=0, high=0.5, size=(testing_no_negative_features,))
testing_dummy_positive_features = np.reshape(testing_dummy_positive_features, (len(testing_dummy_positive_features), 1))
testing_dummy_negative_features = np.reshape(testing_dummy_negative_features, (len(testing_dummy_negative_features), 1))
testing_dummy_positive_labels = np.ones((testing_dummy_positive_features.shape))
testing_dummy_negative_labels = np.zeros((testing_dummy_negative_features.shape))

testing_dummy_features = np.concatenate((testing_dummy_positive_features, testing_dummy_negative_features))
print(f'len: {len(testing_dummy_features)}')
testing_dummy_labels = np.zeros(testing_no_of_nodes)
np.random.shuffle(testing_dummy_features)
for j in range(len(testing_dummy_features)):
    if testing_dummy_features[j] >= 0.5:
        testing_dummy_labels[j] = 1.0
    else:
        testing_dummy_labels[j] = 0.0
...
# step | distance | run distance | calories | heartbeat | sleep | spo2    
train_graph_2.ndata['h'] = torch.from_numpy(testing_dummy_features).float()

train_graph_2 = dgl.add_self_loop(train_graph_2)

# cardiovascular | skin | gastrointestinal symptoms | eye, ears, nose, throat symptoms | musculoskeletal | neuropsychiatric symptoms | respiratory | genitourinary, reproductive | systemic
train_graph_2.ndata['labels'] = torch.from_numpy(testing_dummy_labels).float()
# model1_2 = GCN(7, 1)
