...
pos_x = np.random.uniform(low=0.5, high=1, size=(2,))
neg_x = np.random.uniform(low=0, high=0.5, size=(5,))
...
for i in range(k):
    ...
    train_graph = Data(x = torch.from_numpy(dummy_features).float(), edge_index = train_edge_index, y = torch.from_numpy(dummy_labels).float())
    ...
    validation_graph = Data(x = torch.from_numpy(validation_dummy_features).float(), edge_index = validation_edge_index, y = torch.from_numpy(validation_dummy_labels).float())
